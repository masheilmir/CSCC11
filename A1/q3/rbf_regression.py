...
class RBFRegression():
    ...
    def predict(self, X):
        ...
        B = np.ones((X.shape[0], 1)) # creates ones matrix

        for k in range(0, self.K): # performs function on each X and puts into one array
            bFormula = self._rbf_2d(X, k)
            B = np.column_stack((B, bFormula)) # appending each b_k(x) columnwise

        return B @ self.parameters # matrix mult B Matrix with weights matrix
    ...
    def fit_with_l2_regularization(self, train_X, train_Y, l2_coef):
        ...
        B = np.ones((train_X.shape[0], 1)) # creates ones matrix

        for k in range(0, self.K): # performs function on each X and puts into one array
            bFormula = self._rbf_2d(train_X, k)
            B = np.column_stack((B, bFormula)) # appending each b_k(x) columnwise

        idMat = np.eye(self.K + 1)
        invMat = np.linalg.inv((B.T @ B) + l2_coef*idMat)
        ByMat = B.T @ train_Y
        w = invMat @ ByMat 
        self.parameters = w
